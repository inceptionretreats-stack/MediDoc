...
## 2026-06-22 - Phase 0: Supabase database goes live
**Built**
- Linked Supabase CLI to dedicated project `xwnrjpmktffnrldhkayf` (org `uxrkydqrcwuljilsvupc`, "MediDoc", region `ap-south-1`, Postgres 17.6.1). Resolves the long-standing free-project-limit blocker by using a new org.
- Pushed all four migrations via `supabase db push`:
  - `20260616090000_init.sql` — extensions, enums, `set_updated_at()` trigger.
  - `20260616090100_tenancy_rbac.sql` — `tenants`, `profiles`, `user_roles`, RLS helpers (`is_platform_admin`, `is_super_admin`, `is_tenant_member`, `has_tenant_role`).
  - `20260616090200_directory.sql` — `specialties`, `clinics`, `doctors`, `doctor_specialties`, `listings`, `availability_slots`.
  - `20260616090300_leads_bookings.sql` — `leads`, `lead_events`, `bookings`, `reviews`, `audit_log`.
- Seeded the 10 base specialties via new `scripts/seed-specialties.mjs` (uses service-role key + upsert with `onConflict: slug`; idempotent).
- Regenerated `lib/types/database.ts` from the live schema (`supabase gen types typescript --project-id …`). 896 lines, typecheck passes clean.
- Wrote `scripts/verify-schema.mjs` — runs three checks: every table is queryable with service role, anon can read `specialties`, anon is RLS-blocked from `tenants`. All three passed.
- Fixed `.mcp.json` filesystem MCP server path: was `f:\Doctor` (legacy drive), now `c:\web applications\Doctor\Doctor` (current project root).
- Snapshot of working tree frozen to `.versions/v4-db-live/` (2311 files; excluded `node_modules`, `.next`, `.git`, logs, build info).

**Verification**
- `npm run typecheck` passed after regenerated types.
- `scripts/verify-schema.mjs` returned 14/14 tables reachable, 10/10 specialties readable as anon, 0 tenants visible as anon (RLS deny confirmed).
- Confirmed in Supabase project status: `ACTIVE_HEALTHY`.

**Issues + fixes**
- First attempt at `supabase gen types … | Out-File -NoNewline` collapsed all newlines into a single 1-line file (PowerShell pipeline behavior). Re-ran capturing into `$types = … | Out-String` then writing with `[System.IO.File]::WriteAllText` and UTF-8 (no BOM) — produced the correct 896-line file.

**Deferred / still pending in Phase 0**
- Mirror `.env.local` keys into Vercel project env (Production + Preview): requires `vercel login` (current shell session is not authorized). Keys to push: `NEXT_PUBLIC_SUPABASE_URL`, `NEXT_PUBLIC_SUPABASE_ANON_KEY`, `SUPABASE_SERVICE_ROLE_KEY`, `NEXT_PUBLIC_APP_URL`. Without these, a new Vercel deploy will not read the live DB — but local dev is fully unblocked.

**Blocked / next**
- Phase 1 (Auth + middleware + RBAC enforcement) can now start. See plan at `C:\Users\Dell\.claude\plans\dreamy-mixing-book.md`.

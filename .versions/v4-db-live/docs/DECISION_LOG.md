...
### 2026-06-17 - Multi-agent v3 Vercel deployment
- **Decision:** deploy the verified multi-agent v3 build with the Vercel CLI to project
  `doctor-platform` (`prj_TW5frDxjA3OEI5RigIrbS3POFpsg`) under team `thewebspell`
  (`team_J6RRVVaSLMKbKLvxf3beewaN`). **Why:** the CLI project link is the production deployment
  scope, while the Vercel connector is authorized to a different team and cannot inspect this
  project. **By:** Codex.
- **Decision:** keep Supabase deployment blocked instead of applying migrations to `salespilot`.
  **Why:** `salespilot` has an unrelated schema and a conflicting `public.leads` table; Doctor
  Platform needs a dedicated or explicitly approved reviewed project path. **By:** Codex.

### 2026-06-17 - Multi-agent local UI completion
- **Decision:** use parallel agents for independent local UI slices: booking/notifications,
  role workspaces, and AI-assist previews. **Why:** the user requested multi-agent mode and these
  workstreams had mostly disjoint routes/components. **By:** Codex.
- **Decision:** keep booking, roles, notifications, and AI pages fixture-backed with explicit
  implementation-boundary copy. **Why:** live DB mutations, Auth/RLS role scoping, provider
  integrations, and AI Gateway prompts remain schema/security/AI-owned work. **By:** Codex.
- **Decision:** snapshot the integrated multi-agent UI state into `.versions/v3/`. **Why:** preserve
  a recoverable checkpoint after adding three new production routes. **By:** Codex.

### 2026-06-17 - Phase 1 admin operations slice
- **Decision:** implement `/admin` as a fixture-backed local operations dashboard instead of live
  admin mutations. **Why:** Codex can safely complete the Admin/Super Admin UX surface while
  preserving the server/RLS boundary for verification, moderation, audit writes and cross-tenant
  visibility. **By:** Codex.
- **Decision:** remove the custom 420px action-button breakpoint and keep key action rows stacked
  until the standard `sm` breakpoint. **Why:** prevents narrow mobile clipping and keeps controls
  stable on low-width Android devices. **By:** Codex.
- **Decision:** freeze the post-admin local UI state into `.versions/v2/`. **Why:** follows the
  snapshot discipline before continuing to the next meaningful phase. **By:** Codex.
